...
| `en-vivo.html` | **BORRADOR (noindex, no enlazada)**: transmisión en vivo por sede + asistencia virtual verificada (folio, latidos, comprobaciones, códigos). Diseño en `docs/en-vivo-asistencia.md`; backend de referencia en `apps-script/Asistencia.gs` |
...
- **Backend (director, cuenta CUCEA)**: pegar `Asistencia.gs` + sustituir `Code.gs`, correr `instalarPlaticasIV()` e `instalarDisparadorJornadaVirtual()`, bajar `meta_horas_valor_curricular`, nueva versión del deployment. Ver `docs/en-vivo-asistencia.md` §5.

- **Publicar `en-vivo.html`** cuando el director lo pida (tras sus pruebas de Zoom/YouTube): rellenar `STREAMS`, `MODO_PRUEBA = false`, quitar noindex/franja/`hidden`, sitemap. Pasos en `docs/en-vivo-asistencia.md` §5.
